...
def main():
    ...
    # Get current version
    djangofmt_dep = Requirement(pyproject["project"]["dependencies"][0])
    current_version = Version(list(djangofmt_dep.specifier)[0].version)

    # Get newer versions from PyPI
    resp = urllib3.request("GET", "https://pypi.org/pypi/djangofmt/json")
    if resp.status != 200:
        raise RuntimeError
...
def update_version_in_files(version: Version) -> tuple[str, ...]:
    def replace_pyproject_toml(content: str) -> str:
        return re.sub(r'"djangofmt==.*"', f'"djangofmt=={version}"', content)
